...
workflow ENA_SUBMISSION_PREP {

    take:
    // The depth TSV is this run's own coverage measurement, carried here rather
    // than read back out of mitogenome_data. That is what lets this subworkflow
    // run with --skip_upload_results: it no longer depends on a row this run
    // writes. Pass assets/placeholders/empty_mito_depth.tsv where there is no
    // measurement; PREPARE_ENA_METADATA then falls back to the stored value.
    mitogenome_qc // tuple val(meta), val(species_name), val(proceed_qc true/false), val(circular true/false), path(annotation/*), path(mito_depth.tsv)
    ...
    ch_ena_named = mitogenome_qc.map { meta, species, proceed, circular, files, depth_tsv ->
        ...
        [ meta_ext, species, proceed, circular, files, depth_tsv ]
    }

    ch_circular = ch_ena_named.map { meta, _species, _proceed, circular, _files, _depth -> [ meta, circular ] }
    ch_format_input = ch_ena_named.map { meta, species, proceed, _circular, files, _depth -> [ meta, species, proceed, files ] }
    ...
    PREPARE_ENA_METADATA(
        ch_ena_named.map { meta, _species, _proceed, _circular, _files, depth_tsv -> [ meta, depth_tsv ] },
        sql_config_file
    )
    ...
    ch_table2asn_held = ch_table2asn_branched.fail
    ...
    // Totality over everything that ENTERED this subworkflow.
    //
    // The table2asn fragment above only covers samples that got as far as the
    // table2asn verdict. A sample that leaves earlier emits nothing at all and
    // vanishes: it appears in neither held_samples.tsv nor the submission-ready set,
    // and COMPILE_HELD_SAMPLES can only report it as "unaccounted". That is not
    // hypothetical -- in the 20-sample invert panel FOUR QC-passing samples
    // (INV03_ZOANTHUS, INV11_EUCRATE, INV13_SEMIBALANUS, INV15_ACANTHASTER) did
    // exactly this when BUILD_SOURCE_MODIFIERS exited 1. It has no explicit
    // errorStrategy, so it inherits the global 'ignore', emits no src_file, and the
    // inner join at ch_processed_files silently drops the sample.
    //
    // Fixed with a remainder join over the whole subworkflow input rather than an
    // errorStrategy change on that one process: catching only BUILD_SOURCE_MODIFIERS
    // would leave every sibling step with the identical hole. The reason names the
    // last stage the sample was seen at, so the report points at the actual failure
    // instead of just asserting one happened.
    //
    // Keyed on meta.id (not the whole meta map) because the metas here have been
    // extended with ENA fields since ch_ena_named was built, so map identity would
    // not match. Resolves at channel close, which is when COMPILE_HELD_SAMPLES runs
    // anyway.
    ch_prep_entered = ch_ena_named.map { meta, _sp, _pr, _ci, _f, _d -> [ meta.id, meta ] }
    ch_prep_reached = GEN_FILES_TABLE2ASN.out.gbf_file.map { meta, _gbf -> [ meta.id, 'table2asn' ] }
    ch_prep_src     = BUILD_SOURCE_MODIFIERS.out.src_file.map { meta, _src -> [ meta.id, 'src' ] }
    ch_prep_fmt     = FORMAT_FILES.out.processed_files.map { it -> [ it[0].id, 'format' ] }

    ch_pre_submission_held = ch_prep_entered
        .join(ch_prep_reached, by: 0, remainder: true)
        .join(ch_prep_src,     by: 0, remainder: true)
        .join(ch_prep_fmt,     by: 0, remainder: true)
        .filter { _id, meta, reached, _src, _fmt -> meta != null && reached == null }
        .collectFile { _id, meta, _reached, src, fmt ->
            // Walk the chain backwards to the last stage that DID emit for this sample;
            // the stage named is therefore the first one that did not, i.e. the one that
            // dropped it. Ordering matters: a sample with a .src got past
            // BUILD_SOURCE_MODIFIERS, one with only processed_files did not.
            def stage = src ? 'GEN_FILES_TABLE2ASN'
                            : (fmt ? 'BUILD_SOURCE_MODIFIERS' : 'FORMAT_FILES')
            [ "${meta.mt_assembly_prefix}.PRE_SUBMISSION.held.tsv",
              "${meta.id}\t${meta.mt_assembly_prefix}\tPRE_SUBMISSION\t" +
              "ena_prep_failed: ${stage} produced no output (task failed and was ignored)\n" ]
        }

    ch_held_fragments = ch_table2asn_held.mix(ch_pre_submission_held)
    ...
    held_fragments          = ch_held_fragments            // channel: path(<prefix>.held.tsv) — table2asn quarantines plus anything that never reached table2asn
