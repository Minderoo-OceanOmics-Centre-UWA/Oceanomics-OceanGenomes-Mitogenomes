...
workflow QC_ONLY_FROM_ANNOTATIONS {
    ...
    // Parse the class -> genetic code map before anything resolves a code from it.
    // prepare_samplesheet does this at the top of its own workflow; this entrypoint
    // never runs that subworkflow, and MitoGeneticCode.forClass() raises rather than
    // quietly returning null for every class if the asset was not loaded.
    ...
    // re-QCing an invertebrate here silently rewrote its annotation to code 2 and
    // submitted it that way -- a coral annotated under code 4, a mollusc or crustacean
    // under 5, a tunicate under 13, all flattened to the vertebrate table by a step
    // that was only meant to re-check them.
    //
