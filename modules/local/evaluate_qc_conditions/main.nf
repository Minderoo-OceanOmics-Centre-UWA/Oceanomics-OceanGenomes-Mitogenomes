process EVALUATE_QC_CONDITIONS {
    ...
    tuple val(meta), path("held_reason.txt"), emit: reason
    ...
    def effective_args = "--blast-table ${blast_table} --annotation-csv ${annotation_csv} --circularity-check ${circularity_check} --circular ${circular_flag} --output-species species_name.txt --output-proceed proceed_qc.txt --output-circular circular.txt --output-reason held_reason.txt --output-versions versions.yml"
    """
