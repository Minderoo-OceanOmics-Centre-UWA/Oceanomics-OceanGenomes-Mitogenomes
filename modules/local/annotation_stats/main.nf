// Compile the per-assembly annotation statistics: gene/CDS/tRNA/rRNA counts, the
// missing-gene set, gene order and the ORF verdict for every PCG.
//
// This is PURE LOCAL COMPUTATION and deliberately has no database config input. It
// used to be the first half of PUSH_MTDNA_ANNOTATION_RESULTS, which meant the only
// way to learn whether a mitogenome was any good was to also write it to
// PostgreSQL: a run with --skip_upload_results (or no --sql_config at all, which is
// how the invertebrate work is developed) produced no gene counts, no completeness
// verdict, and an assembly summary with its annotation columns empty. The QC half is
// separated so it can run unconditionally; the DB write is now its own process.
process ANNOTATION_STATS {
    tag "$meta.id"
    ...
    tuple val(meta), path("12_annotation_stats.tool_params_mqcrow.html"), emit: tool_params
    ...
    def effective_args = "annotation_stats.py ${args} ${class_arg} ${gcode_arg} ${family_arg} ${order_arg} ${genus_arg} ${lca_arg} *.gff proteins"
    """
    ...
    cat <<-END_TOOL_PARAMS > 12_annotation_stats.tool_params_mqcrow.html
    <tr><td>Annotation Statistics</td><td><samp>${effective_args}</samp></td><td>Calculates annotation statistics for ${meta.id}.</td></tr>
    END_TOOL_PARAMS
    ...
    stub:
    def args = task.ext.args ?: ''
    def effective_args = "annotation_stats.py ${args} *.gff proteins"
    """
